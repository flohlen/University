...
def qsort_cutoff(liste):  
	...
	return qsort_cutoff(linke)+liste[pivot:pivot+1]+qsort_cutoff(rechte)

def qsort_intro(liste):  
	global vergleiche

	def qsort_intro_help(liste,schranke):
		global vergleiche
		if schranke == 0:
		...
		return qsort_intro_help(linke,schranke-len(liste))+liste[pivot:pivot+1]+qsort_intro_help(rechte,schranke-len(liste))

	return qsort_intro_help(liste,2*math.log(len(liste),2))			
...
def qsort_random(liste):  
	...
	ran_elemenet = random.choice(list(enumerate(liste)))
	pivot = ran_elemenet[0]
